...
use crate::sc::spatial_index::SpatialIndex;
use crate::sc::types::*;
...
impl SurfaceGenerator {
	...
	pub fn assign_attention_numbers(&mut self) {
		...
		if self.settings.use_spatial_index {
			let mol0: Vec<usize> = (0..atoms_len).filter(|&i| self.run.atoms[i].molecule == 0).collect();
			let mol1: Vec<usize> = (0..atoms_len).filter(|&i| self.run.atoms[i].molecule == 1).collect();
			let index0 = SpatialIndex::new(mol0, self.settings.separation_cutoff, |i| self.run.atoms[i].coor);
			let index1 = SpatialIndex::new(mol1, self.settings.separation_cutoff, |i| self.run.atoms[i].coor);
	...
	fn compute_neighbors_all_parallel(&mut self) -> Result<(), SurfaceCalculatorError> {
		...
		let atom_index = if self.settings.use_spatial_index {
			Some(SpatialIndex::new(0..len, 2.0 * radmax + 2.0 * rp, |i| atoms[i].coor))
		} else {
			None
		};
	...
	fn generate_contact_surface_parallel(&mut self) -> Result<(), SurfaceCalculatorError> {
		...
		let atoms: &Vec<Atom> = &self.run.atoms;
		let radmax = self.run.radmax;
		let use_spatial_index = self.settings.use_spatial_index;
		let mol0: Vec<usize> = (0..atoms.len()).filter(|&i| atoms[i].molecule == 0).collect();
		let mol1: Vec<usize> = (0..atoms.len()).filter(|&i| atoms[i].molecule == 1).collect();
		let index0 = SpatialIndex::new(mol0, radmax + rp, |i| atoms[i].coor);
		let index1 = SpatialIndex::new(mol1, radmax + rp, |i| atoms[i].coor);
