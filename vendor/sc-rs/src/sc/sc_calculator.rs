...
impl ScCalculator {
	pub fn new() -> Self { Self { base: SurfaceGenerator::new() } }
	...
	fn trim_peripheral_band(&mut self, i: usize) -> Result<ScValue, SurfaceCalculatorError> {
		let (indices, area) = if self.base.settings.enable_parallel {
			...
			let unburied_index = if self.base.settings.use_spatial_index {
				Some(SpatialIndex::new(
					(0..sdots.len()).filter(|&idx| !sdots[idx].buried),
					self.base.settings.peripheral_band,
					|idx| sdots[idx].coor,
				))
			} else {
				None
			};
			let peripheral_band = self.base.settings.peripheral_band;
			let peripheral_band2 = peripheral_band * peripheral_band;
			let indices: Vec<usize> = (0..sdots.len()).into_par_iter()
				.filter(|&idx| {
					...
					if let Some(index) = &unburied_index {
						...
						self.trim_peripheral_band_check_dot(idx, sdots)
					}
				})
				.collect();
	...
	fn trim_peripheral_band_check_dot(&self, dot_index: usize, sdots: &[Dot]) -> bool {
		...
		for (i, dot2) in sdots.iter().enumerate() {
			...
			if dot.coor.distance_squared(dot2.coor) <= r2 { return false; }
		}
		true
	}
	...
	fn calc_neighbor_distance(&mut self, my: usize, their: usize) {
		let (distances, scores, distmin_sum, score_sum) = if self.base.settings.enable_parallel {
			...
			let their_index = if self.base.settings.use_spatial_index {
				Some(SpatialIndex::new(their_dots.iter().copied(), 2.0, |idx| run_ref.dots[their][idx].coor))
			} else {
				None
			};
